...
6. 如果用户明确要求创建 / 继续 project，再读取 `protocols/project.md`，更新 project page，并在 delegate/project frontmatter 中互链。
7. 写 `# Response` 时先展开用户在意的交付内容；把运行监控、命令、完整路径、对比口径、重试/联网/缓存等明细放进默认收起的 Obsidian callout，例如 `> [!note]- 运行与验证明细`。
8. 交付前判断是否需要人类 review：如果需要人类确认、批准、决策、补资料，或原始目标尚未完成，设为 `status: need_review`。
9. 如果 agent 已完成原始目标，且没有 `Needs Review`、没有 `Next`、没有阻塞或待人类决策事项，设为 `status: done`。
10. 只有人类勾选 `archive: true` 后，才设置 `archived`。
...
- 不让 project page 替代 delegate；project 只能作为明确项目请求的附加记录。
